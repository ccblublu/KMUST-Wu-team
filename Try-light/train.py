...
Usage:
    $ python path/to/train.py --data coco128.yaml --weights                                                                                                                                                       
"""
import argparse
import math
import os
...
LOCAL_RANK = int(os.getenv(
    'LOCAL_RANK', -1))  # https://pytorch.org/docs/stable/elastic/run.html  /home/zzd/ZZD/ZZDpy/SW-light/train.py
...
def train(
        ...
        device,
        callbacks):
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, seg_iou = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.seg_iou
    ...
    # Freeze
    Encoder_para_idx = [str(i) for i in range(0, 18)]
    Det_Head_para_idx = [str(i) for i in range(18, 26)]
    Da_Seg_Head_para_idx = [str(i) for i in range(26, 49)]
    freeze = [f'model.{x}.' for x in range(freeze)]  # layers to freeze
    # for k, v in model.named_parameters():
    #       v.requires_grad = True  # train all layers
    #       if any(x in k for x in Encoder_para_idx):
    #       #if any(x in k for x in Da_Seg_Head_para_idx):
    ...
    scheduler = lr_scheduler.LambdaLR(
        optimizer,
        lr_lambda=lf)  # 
    # plot_lr_scheduler(optimizer, scheduler, epochs, './runs')
    ...
    # Model attributes
    nl = de_parallel(
        model).model[detect_layers].nl  # number of detection layers (to scale hyps)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss(
                    pred, masks.to(device),
                    targets.to(device), early_stop_seg)  # loss scaled by batch_size
                if RANK != -1:
            ...
            if RANK in [-1, 0]:
                ...
                callbacks.run('on_train_batch_end', ni, model, imgs, masks,
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val.run(data_dict,
                                           batch_size=bs, #batch_size // WORLD_SIZE * 2,
                                           imgsz=imgsz,
                                           model=ema.ema,
                                           single_cls=single_cls,
                                           dataloader=val_loader,
            ...
            # cal weather stop seg
            cur_seg_iou = results[4]
            # if cur_seg_iou > seg_iou:
            #     for k, v in model.named_parameters():
            #         v.requires_grad = True  # train all layers
            #         if any(x in k for x in Encoder_para_idx + Da_Seg_Head_para_idx):
            #             LOGGER.info(f'freezing {k}')
            #             v.requires_grad = False
            # if epoch > 100:
            #     early_stop_seg = 1
            # Update best mAP
                ...
                1,
                -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95, IoU]
            if fi > best_fitness:
                best_fitness = fi
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='/home/zzd/ZZD/ZZDpy/SW-light/models/SW-light31.yaml', help='model.yaml path')
    parser.add_argument('--data',
                        type=str,
                        default=ROOT / '/home/zzd/ZZD/ZZDpy/SW-light/data/VOC.yaml',
    ...
    parser.add_argument('--seg-iou',
                        type=float,
                        default=0.90 ,
                        help='the threshold to early stop seg branch')
    ...
    parser.add_argument('--SGD',
                        action='store_true',
                        help='use torch.optim.SGD() optimizer')
    ...
    parser.add_argument('--freeze',
                        type=int,
                        default=0,
                        help='Number of layers to freeze. backbone=10, all=27')
    parser.add_argument(
        '--save-period',
        type=int,
        default=-1,
...
if __name__ == "__main__":
    ...
    main(opt)
    import json
    from utils.datasets import HASH_TABLE
    HASH_TABLE = sorted(HASH_TABLE)
    obj_ = json.dumps(HASH_TABLE, indent=4)
    test_write_dir = "./buglog.json"
    with open(test_write_dir, "w") as f:
        f.write(obj_)
