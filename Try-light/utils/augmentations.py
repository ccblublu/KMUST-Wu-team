    ...
    def __init__(self):
        ...
        try:
            ...
            self.transform = A.Compose([
                A.Blur(p=0.01), #模糊化
                A.MedianBlur(p=0.01),#另外一种模糊化
                A.ToGray(p=0.01),#灰度图
                A.CLAHE(p=0.01),#锐化？
                A.RandomBrightnessContrast(p=0.0),#随机改变输入图像的亮度和对比度
                A.RandomGamma(p=0.0), #对比度
                A.ImageCompression(quality_lower=75, p=0.0)], #减少图像的Jpeg、WebP压缩。
...
def copy_paste(im, mask, labels, segments, p=0.5):
    ...
    if p and n:
        ...
        mask[i] = 0
    return im, mask, labels, segments
