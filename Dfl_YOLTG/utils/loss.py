...
import numpy as np
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy,MPDIoU
...
#         self.overall_weight = torch.tensor(loss_weight).to(device)
#         self.bce_loss = nn.CrossEntropyLoss(                       #CrossEntropyLoss 交叉熵损失函数  
...
#         masked_target =masked_target.unsqueeze(1).float()
...
#         ce_loss = self.bce_loss(masked_x, masked_target)   #还是这一步，计算交叉熵损失的时候损失没有了
#         # pred = F.softmax(x_perm, dim=1)
#         # pred = torch.clamp(pred, min=1e-5, max=1.0)
#         # label_indices = torch.argmax(target, dim=1)  # 获取每个位置上的最大值所对应的索引
#         # label_one_hot = F.one_hot(label_indices, 2).float().to(target.device)
...
#         # label_one_hot = label_one_hot.transpose(1, 3).transpose(2, 3)
...
class CEL_Masked_Loss(nn.Module):
    ...
    def forward(self, x, target):
        ...
        masked_x = x_perm[mask_input].view(-1, x_perm.shape[-1])  # (N, C)
        # masked_target = (masked_target>100).long()
        #ce_loss = self.cross_entropy(masked_x.float(), masked_target.float())
        ce_loss = self.cross_entropy(masked_x, masked_target.to(torch.int64))
        pred = F.softmax(x_perm, dim=1)
        pred = torch.clamp(pred, min=1e-5, max=1.0)
        label_one_hot = F.one_hot(target.long(), 2).float().to(target.device)#hpy=3
        label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0) #最小设为 1e-4，即 A 取 -4
        rce = (-1 * torch.sum(pred * torch.log(label_one_hot), dim=1))
        loss = self.a * ce_loss + self.b * rce.mean()
        return loss
...
class BCEBlurWithLogitsLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        true = true.to(torch.float16)
        loss = self.loss_fcn(pred, true)
        pred = torch.sigmoid(pred)  # prob from logits
        dx = pred - true  # reduce only missing label effects
        # dx = (pred - true).abs()  # reduce missing label and false label effects
        alpha_factor = 1 - torch.exp((dx - 1) / (0.05 + 1e-4))
        loss *= alpha_factor
        return loss.mean()
...
def seg_distillation_loss(student_output, teacher_output, target, alpha=0.1, temperature=5.0):
    """
    student_output: 学生模型的输出, shape [batch_size, num_classes, height, width]
    teacher_output: 教师模型的输出, shape [batch_size, num_classes, height, width]
    target: 真实标签, shape [batch_size, height, width]
    alpha: 损失函数中的权重系数
    temperature: 温度参数
    """
    # Soft target loss
    ...
    soft_loss = soft_loss / (soft_loss.max()+ 1e-6)
    # 将 target 从 [batch_size, height, width] 变为 [batch_size, num_classes, height, width]
    # 进行 one-hot 编码
    target_one_hot = F.one_hot(target, num_classes=student_output.size(1)).permute(0, 3, 1, 2).float()
    
    # Hard target loss
    hard_loss = F.binary_cross_entropy_with_logits(student_output, target_one_hot)
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        CELmask = CEL_Masked_Loss(device,[h['seg_weight1'], h['seg_weight2']])
        ...
        det = model.module.model[detection_head] if is_parallel(model) else model.model[detection_head]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance,self.CELmask = BCEcls, BCEobj, model.gr, h, autobalance,CELmask
        for k in 'na', 'nc', 'nl', 'anchors':
            setattr(self, k, getattr(det, k))

    def __call__(self, p, mask,targets, teacher_det=None,teacher_seg = None ,student_det =None, student_seg = None,seg_choose = 1, is_training =True):  # predictions, targets, model
        ...
        for i, pi in enumerate(p[0]):  # layer index, layer predictions
            ...
            if n:
                ...
                #iou = MPDIoU(pbox.T, tbox[i], x1y1x2y2=False)
        ...
        rust_p = p[1]
        rust_gt = mask#.unsqueeze(1).repeat(1,2,1,1)
        lseg += self.CELmask(rust_p,rust_gt)
    ...
    # Only compute distillation losses during training
        if is_training:
            detect_loss = 0  # Initialize detect_loss as tensor
            seg_loss = 0  # Initialize seg_loss as tensor
            for student_feat, teacher_feat in zip(p[0], teacher_det):
                smooth_l1_loss = nn.SmoothL1Loss()
                detect_loss = smooth_l1_loss(student_feat, teacher_feat)
                #detect_loss += nn.SmoothL1Loss(student_feat, teacher_feat)
            detect_loss = detect_loss.unsqueeze(0) * 0.2
            seg_loss = seg_distillation_loss(p[1], teacher_seg, mask).unsqueeze(0) * 0.2 # Ensure seg_loss is calculated
        else:
            detect_loss = torch.zeros(1, device=device)  # Initialize detect_loss as tensor
            seg_loss = torch.zeros(1, device=device)  # Initialize seg_loss as tensor

        loss = lbox + lobj + lcls + lseg + detect_loss + seg_loss
        return loss * bs, torch.cat((lbox, lobj, lcls, lseg, detect_loss, seg_loss)).detach()
...
def imitation_loss(teacher, student, mask_TS):
    if student is None or teacher is None:
        return 0
    # print(teacher.shape, student.shape, mask.shape)
    diff = torch.pow(student - teacher, 2) * mask_TS
    diff = diff.sum() / mask_TS.sum() / 2
...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))
        CELmask = CEL_Masked_Loss(device,[h['seg_weight1']])
        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
        self.cp, self.cn = smooth_BCE(eps=h.get('label_smoothing', 0.0))  # positive, negative BCE targets
    ...
    def __call__(self, p, mask,targets,teacher=None, student=None, mask_TS=None):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj,lseg = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device),torch.zeros(1, device=device)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #iou = MPDIoU(pbox.T, selected_tbox, x1y1x2y2=False)
                lbox += (1.0 - iou).mean()  # iou loss
        ...
        dmask = imitation_loss(teacher, student,mask_TS)*0.01
        teacher = teacher * mask_TS
        student = student * mask_TS
        smak = F.mse_loss(student, teacher)
        loss = lbox + lobj + lcls + lseg + dmask + smak
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ...
                #iou_aux =MPDIoU(pbox_aux.T, selected_tbox_aux, x1y1x2y2=False)
