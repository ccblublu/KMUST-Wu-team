...
def fast_hist(label_pred, label_true, num_classes):
    hist = np.bincount(label_true.astype(int) + label_pred, minlength=num_classes)
    hist = np.expand_dims(hist, axis=(0, 1))
    return hist[:,:,0]
...
class Metric_mIoU():
    ...
    def update(self, predict, target):    #应该是shape[1,640,640]
        predict, target = converter(predict), converter(target)

        self.hist += fast_hist(predict, target, self.class_num)

    def get_miou(self):
        denominator = np.sum(self.hist, axis=1) + np.sum(self.hist, axis=0) - np.diag(self.hist)
        denominator[denominator == 0] = 1  # 将分母中的零值替换为1，避免除以零
        iou = np.diag(self.hist) / denominator
        #iou[np.isnan(iou)] = 0.0  # 将无效值替换为0.0
        miou = np.nanmean(iou)  # 计算平均交并比，忽略无效值
        return miou
