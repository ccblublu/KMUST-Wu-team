...
class SegmentationLoss(nn.Module):
    ...
    def forward(self, x, target):
        ...
        # 计算二分类交叉熵损失
        loss = self.cross_entropy(x, target.float())
...
class CEL_Masked_Loss(nn.Module):
    ...
    def __init__(self, device, loss_weight):
        ...
        self.overall_weight = torch.tensor(loss_weight).to(device)
        self.bce_loss = nn.BCEWithLogitsLoss(                       #CrossEntropyLoss 交叉熵损失函数  
    ...
    def forward(self, x, target):
        ...
        label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0) #最小设为 1e-4，即 A 取 -4         #这里所有数据都消失了
