...
class Net(t.nn.Module):
    ...
    def __init__(self):
        ...
        # self.conv1 = qnn.QuantConv2d(1, 32, (5, 5), padding=(2, 2), bias=True, quant_desc_input=tensor_quant.QUANT_DESC_8BIT_PER_TENSOR,quant_desc_weight=tensor_quant.QUANT_DESC_8BIT_CONV2D_WEIGHT_PER_CHANNEL)
        #self.conv2 = t.nn.Conv2d(32, 64, (5, 5), padding=(2, 2), bias=True)
        self.conv2 = qnn.QuantConv2d(32, 64, (5, 5), padding=(2, 2), bias=True)
        # self.conv2 = qnn.QuantConv2d(32, 64, (5, 5), padding=(2, 2), bias=True, quant_desc_input=tensor_quant.QUANT_DESC_8BIT_PER_TENSOR,quant_desc_weight=tensor_quant.QUANT_DESC_8BIT_CONV2D_WEIGHT_PER_CHANNEL)
        #self.fc1 = t.nn.Linear(64 * 7 * 7, 1024, bias=True)
        self.fc1 = qnn.QuantLinear(64 * 7 * 7, 1024, bias=True)
        # self.fc1 = qnn.QuantLinear(64 * 7 * 7, 1024, bias=True, quant_desc_input=tensor_quant.QUANT_DESC_8BIT_PER_TENSOR,quant_desc_weight=tensor_quant.QUANT_DESC_8BIT_LINEAR_WEIGHT_PER_ROW)
        #self.fc2 = t.nn.Linear(1024, 10, bias=True)
        self.fc2 = qnn.QuantLinear(1024, 10, bias=True)
        # self.fc2 = qnn.QuantLinear(1024, 10, bias=True, quant_desc_input=tensor_quant.QUANT_DESC_8BIT_PER_TENSOR,quant_desc_weight=tensor_quant.QUANT_DESC_8BIT_LINEAR_WEIGHT_PER_ROW)
...
for epoch in range(10):
    for xTrain, yTrain in trainLoader:
        ...
        opt.zero_grad()
        y_, z = model(xTrain)
        loss = ceLoss(y_, yTrain.argmax(1))
        loss.backward()
        opt.step()
...
# 进入验证阶段，设置当前网络状态 “disable_quant”， “enable_calib”
with t.no_grad():
...
#此步很关键，polygraphy生成的onnx推理速度会有较大的提升（猜测此处才是真正完成了量化？）
