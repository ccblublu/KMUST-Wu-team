...
class LSKblock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = nn.Conv2d(dim, dim, 13, padding=6, groups=dim)
...
class LC(nn.Module):
    def __init__(self, in_channels, out_channels, block_size=4):
        ...
        self.conv1 = DWConv(in_channels, out_channels, kernel_size=3, padding=1)
        self.conv2 = DWConv(out_channels, out_channels, kernel_size=3, padding=1)
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.downsample = nn.MaxPool2d(kernel_size=2, stride=2)
        # self.concat = nn.Conv2d(out_channels * (block_size ** 2), out_channels, kernel_size=1)
        self.normal_conv1 = DWConv(in_channels, out_channels, kernel_size=3, padding=1)
...
class DKC(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dw_conv(x)
        x = x + self.dw_conv2(x) + self.dw_conv3(x) + self.dw_conv4(x)
        x = self.pw_conv(x)
        y_caa = self.caa_factor(y)
        y_caa = y + y_caa
        attn = x + y_caa
        attn = self.post_conv(attn)
...
class MambaLayer(nn.Module):
    def __init__(self, input_dim, output_dim, d_state = 16, d_conv = 4, expand = 2,):
        ...
        self.mlp = Mlp(in_features=input_dim, hidden_features=output_dim, act_layer=nn.GELU, drop=0. )
    
    def forward(self, x):
        #x = x.to(torch.float32) if x.dtype == torch.float16 else x
        B, C = x.shape[:2]
        ...
        x_mamba1 = self.mamba(x_norm)
        x_mamba1 = x_mamba1.to(x.device)
        x_mamba =  x_mamba1 + x_flat  #nlp格式

        x_mamba_b = self.norm1(x_mamba)  #nlp格式
        x_mamba_b = x_mamba_b.transpose(-1, -2).reshape(B, self.output_dim, *img_dims)  #cv格式
        x_mamba_b = self.mlp(x_mamba_b) #cv格式

        x_mamba = x_mamba.transpose(-1, -2).reshape(B, self.output_dim, *img_dims)  #cv格式

        out = x_mamba + x_mamba_b
        #out = self.proj(out)
...
class MambaLayer_L(nn.Module):
    def __init__(self, input_dim, output_dim, d_state = 16, d_conv = 4, expand = 2,):
        ...
        self.proj = nn.Linear(input_dim, output_dim)
        self.skip_scale = nn.Parameter(torch.ones(1))
        self.mlp = Lo2(in_features=input_dim, hidden_features=output_dim, act_layer=nn.GELU, drop=0. )
...
def get_mamba_layer(in_channels: int, out_channels: int, stride: int=1):
...
def get_mamba_layer_L(in_channels: int, out_channels: int, stride: int=1):

    mamba_layer = MambaLayer_L(input_dim=in_channels, output_dim=out_channels)
    if stride != 1:
        return nn.Sequential(mamba_layer, nn.MaxPool2d(kernel_size=stride, stride=stride))
...
class RMB(nn.Module):
    def __init__(
        ...
        #self.ml = MambaLayer(input_dim=in_channels,output_dim=out_channels)
        self.ml = get_mamba_layer(in_channels=in_channels,out_channels=out_channels)
...
class RMB_L(nn.Module):
    def __init__(
        ...
        super(RMB_L, self).__init__()
        #self.ml = MambaLayer(input_dim=in_channels,output_dim=out_channels)
        self.ml = get_mamba_layer_L(in_channels=in_channels,out_channels=out_channels)
...
class Lo2(nn.Module):
    def __init__(self, in_features, hidden_features, act_layer=nn.GELU, drop=0., shift_size=5):
        super().__init__()
    ...
    def forward(self, x):   #x bchw
        ...
        x_s = x_cat.reshape(B, C, H * W).contiguous()  #x b c hxw
    ...
    def __init__(self, ch_in, ch_out, stride):
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                depthwise_conv(ch_in, ch_in, kernel_size=3, stride=self.stride, padding=1),  # 深度可分离卷积层
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),  # 批归一化层
            nn.ReLU(inplace=True),  # ReLU激活函数
            depthwise_conv(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1),  # 深度可分离卷积层
            nn.BatchNorm2d(branch_features),  # 批归一化层
