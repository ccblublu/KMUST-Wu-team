...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        det = model.module.model[91] if is_parallel(model) else model.model[91]  # Detect() module
    ...
    def __call__(self, p, mask,targets,seg_choose = 1):  # predictions, targets, model
...
class ComputeLossOTA:
    ...
    def __call__(self, p, mask,targets):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj,lseg = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device),torch.zeros(1, device=device)
