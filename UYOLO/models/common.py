...
class DSConv(nn.Module):
    def __init__(self, in_ch, out_ch, morph, kernel_size=3, if_offset=False, extend_scope=2):
        """
    ...
    def forward(self, f):
        ...
        #deformed_feature = deformed_feature.to(device)
        if self.morph == 0:
            x = self.dsc_conv_x(deformed_feature.type(f.dtype))
            x = x.to(deformed_feature.device)
            x = self.gn(x)
            x = self.act(x)
            return x
        else:
...
class DSC(object):
    ...
    def _bilinear_interpolate_3D(self, input_feature, y, x):
        ...
        input_feature_flat = input_feature_flat.to(device)
...
class ASPP(nn.Module):
    def __init__(self, in_channel, depth=128):
        super(ASPP,self).__init__()
        self.mean = nn.AdaptiveAvgPool2d((1, 1)) #(1,1)means ouput_dim
        self.conv = nn.Conv2d(in_channel, depth, 1, 1)
        self.atrous_block1 = nn.Conv2d(in_channel, depth, 3, 1, padding=1, dilation=1)
        self.atrous_block6 = nn.Conv2d(in_channel, depth, 3, 1, padding=8, dilation=8)
        self.atrous_block12 = nn.Conv2d(in_channel, depth, 3, 1, padding=16, dilation=16)
        self.atrous_block18 = nn.Conv2d(in_channel, depth, 3, 1, padding=32, dilation=32)
...
class ASPP1(nn.Module):
    def __init__(self, in_channel, depth=128):
        ...
        self.mean = nn.AdaptiveAvgPool2d((1, 1)) #(1,1)means ouput_dim
        self.conv = nn.Conv2d(in_channel, depth, 1, 1)
        self.atrous_block1 = StripPooling1(in_channel, (20,16))
        self.atrous_block6 = StripPooling4(in_channel, (20,12))
        self.atrous_block12 = StripPooling2(in_channel, (20,8))
        self.atrous_block18 = StripPooling3(in_channel, (20,4))
        self.conv_1x1_output = nn.Conv2d(depth * 5, depth, 1, 1)
...
class StripPooling1(nn.Module):
    """
    Reference:
    """
    def __init__(self, in_channels, pool_size, norm_layer=nn.BatchNorm2d):
        super(StripPooling1, self).__init__()
        self.pool1 = nn.AdaptiveAvgPool2d(pool_size[0])
        self.pool2 = nn.AdaptiveAvgPool2d(pool_size[1])
        self.pool3 = nn.AdaptiveAvgPool2d((1, None))
        self.pool4 = nn.AdaptiveAvgPool2d((None, 1))
...
class StripPooling2(nn.Module):
    ...
    def __init__(self, in_channels, pool_size, norm_layer=nn.BatchNorm2d):
        ...
        self.conv2_2 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                     norm_layer(inter_channels))
        self.conv2_3 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (1, 7), 1, (0, 1), bias=False),
                                     norm_layer(inter_channels))
        self.conv2_4 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (7, 1), 1, (1, 0), bias=False),
                                     norm_layer(inter_channels))
        self.conv2_5 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                     norm_layer(inter_channels),
                                     nn.ReLU(True))
...
class StripPooling3(nn.Module):
    """
    Reference:
    """
    def __init__(self, in_channels, pool_size, norm_layer=nn.BatchNorm2d):
        super(StripPooling3, self).__init__()
        self.pool1 = nn.AdaptiveAvgPool2d(pool_size[0])
        self.pool2 = nn.AdaptiveAvgPool2d(pool_size[1])
        self.pool3 = nn.AdaptiveAvgPool2d((1, None))
        self.pool4 = nn.AdaptiveAvgPool2d((None, 1))
        ...
        self.conv2_2 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                     norm_layer(inter_channels))
        self.conv2_3 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (1, 11), 1, (0, 1), bias=False),
                                     norm_layer(inter_channels))
        self.conv2_4 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (11, 1), 1, (1, 0), bias=False),
                                     norm_layer(inter_channels))
        self.conv2_5 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                     norm_layer(inter_channels),
                                     nn.ReLU(True))
...
class StripPooling4(nn.Module):
    ...
    def __init__(self, in_channels, pool_size, norm_layer=nn.BatchNorm2d):
        ...
        self.conv2_2 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                     norm_layer(inter_channels))
        self.conv2_3 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (1, 5), 1, (0, 1), bias=False),
                                     norm_layer(inter_channels))
        self.conv2_4 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (5, 1), 1, (1, 0), bias=False),
                                     norm_layer(inter_channels))
        self.conv2_5 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                     norm_layer(inter_channels),
                                     nn.ReLU(True))
...
class TConv(nn.Module):
    def __init__(self, dim):
        ...
        self.conv2_1 = nn.Conv2d(dim, dim, (1, 17), padding=(0, 8), groups=dim)
        self.conv2_2 = nn.Conv2d(dim, dim, (17, 1), padding=(8, 0), groups=dim)

        self.conv3_1 = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.conv3_2 = nn.Conv2d(dim, dim, 5, 1, 2, groups=dim)
        self.conv3_3 = nn.Conv2d(dim, dim, 7, 1, 3, groups=dim)
    ...
    def forward(self, x):
        ...
        attn_3_3 = self.conv3_3(attn)
        attn_3 = attn_3_1 + attn_3_2 + attn_3_3
...
class TConvv(nn.Module):
    def __init__(self, dim ):
        super(TConvv,self).__init__()
        self.conv0 = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        self.conv0_1 = nn.Conv2d(dim, dim, (1, 7), padding=(0, 3))
        self.conv0_2 = nn.Conv2d(dim, dim, (7, 1), padding=(3, 0), groups=dim)
        ...
        self.conv3_1 = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.conv3_2 = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.conv3_3 = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.conv4 = nn.Conv2d(dim, dim, 1)
    ...
    def forward(self, x):
        ...
        attn_3_1 = self.conv3_1(attn)
        attn_3_2 = self.conv3_2(attn_3_1)
        attn_3_3 = self.conv3_3(attn_3_2)
        #attn_3 = attn_3_1 + attn_3_2 + attn_3_3

        attn = attn + attn_0 + attn_1 + attn_2 +attn_3_3
...
class SEGB(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.d_model = d_model
        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = TCONV(d_model)
...
class BottleneckT(nn.Module):
    ...
    def forward(self, x):
        return x + self.cv2(x) if self.add else self.cv2(x)
...
class C3T(nn.Module):
    ...
    def __init__(self,
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(BottleneckT(c_, c_, shortcut, g, e=1.0)
                                 for _ in range(n)))
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
