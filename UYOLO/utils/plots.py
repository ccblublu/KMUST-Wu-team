...
from test import calculate_railway_range
# Settings
...
class Annotator:
    ...
    def box_label(self,
        ...
        if self.pil or not is_ascii(label):
            self.draw.rectangle(box, width=8, outline=color)  # box  注释掉这行锚框消失
            if label:
                new_font = ImageFont.truetype(self.font.path,font_size)
                w , h = self.draw.textsize(label,font=new_font)
...
def feature_visualization_origin(x,
                          module_type,
                          stage,
                          n=32,
                          save_dir=Path('runs/val/exp')):
    """
...
def feature_visualization(x,
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            suffix = 1
            while True:
                f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features_{suffix}.png"  # filename
...
def feature_visualization2(x,
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            plt.subplots_adjust(wspace=0.05, hspace=0.05)
            for i in range(n):
                # Apply exponential transformation to enhance brightness
                block = torch.exp(blocks[i])
                ax[i].imshow(block.squeeze())  # cmap='gray'
                ax[i].axis('off')
...
def plot_mask(img, seg):
    ...
    mask_rail = seg == 1
    mask_road = seg == 2
    color_mask[mask_rail] = [255, 0, 0]
    img[mask_rail] = img[mask_rail] * 0.4 + color_mask[mask_rail] * 0.6
    color_mask[mask_road] = [0, 255, 0]
    img[mask_road] = img[mask_road] * 0.4 + color_mask[mask_road] * 0.6
...
def find_narrowest_width(mask_rail):
    # Find contours of the rail mask
    contours, _ = cv2.findContours(mask_rail.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Initialize minimum width and coordinates
    min_width = float('inf')
    min_width_coords = None

    # Iterate through contours to find the narrowest width for each contour
    for contour in contours:
        x, y, w, h = cv2.boundingRect(contour)
        if w < min_width:
            min_width = w
            min_width_coords = (x, y, x + w, y + h)

    return min_width



# def find_median_narrowest_width(mask_rail, num_iterations=100):
#     # Initialize list to store narrowest widths
#     narrowest_widths = []
...
#         # Initialize minimum width for the current iteration
#         min_width = float('inf')
...
#         # Add the narrowest width of the current iteration to the list
#         narrowest_widths.append(min_width)

#     # Calculate the median of narrowest widths
#     median_narrowest_width = np.median(narrowest_widths)

#     return median_narrowest_width


def plot_mask1(img, seg):
    color_mask = np.zeros_like(img, dtype=np.uint8)

    # Create a mask for the rail
    mask_rail = seg == 1
    mask_road = seg == 2
    color_mask[mask_rail] = [255, 0, 0]
    img[mask_rail] = img[mask_rail] * 0.4 + color_mask[mask_rail] * 0.6

    # Find the narrowest width of the rail mask
    num_p = find_narrowest_width(mask_rail)

    # Draw a line on the image to represent the narrowest width
    #if min_width_coords is not None:
        #cv2.line(img, (min_width_coords[0], min_width_coords[1]), (min_width_coords[2], min_width_coords[3]), (0, 255, 0), 2)

    # Create a distance transform of the rail mask
    distance_transform = distance_transform_edt(~mask_rail)

    # Create a mask for the spreading effect
    d = 100 * num_p
    d = d/1.435
    d = int(d)
    mask_spread = (distance_transform <= d).astype(np.bool)

    # Add a faded red region around the rail
    color_mask[mask_spread] = [255, 100, 100]
    img[mask_spread] = img[mask_spread] * 0.4 + color_mask[mask_spread] * 0.6
...
def plot_images(images,
    ...
    for i, (im, seg) in enumerate(zip(images, seg_out)):
        ...
        if seg.max() > 0:
            im = plot_mask(im, seg) #注释掉这两行可以去除分割效果
            # range_mask = calculate_railway_range(seg, im, rito)
            # range_mask[seg==2] = rito # fliter road
            # range_masks[y:y + h, x:x + w] = range_mask
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    #label = f'{cls}' if labels else f'{cls} {conf[j]:.1f}' #注释这里更改是否有标签
                    label = ''
                    annotator.box_label(box, label, color=color)
    annotator.im.save(fname)  # save


def plot_heatmap(cam,
                 images,
                 targets,
                 paths=None,
                 fname='heatmaps.jpg',
    ...
    for i, img in enumerate(images):
        grayscale_cam = cam(img, targets, aug_smooth=True, eigen_smooth=True)
        img_np = deepcopy(img).cpu().float().numpy()
        if np.max(img_np) <= 1:
            img_np *= 255  # de-normalise (optional)
        cam_images = show_cam_on_image(img_np, grayscale_cam, use_rgb=True)
        cam_images = cv2.cvtColor(cam_images, cv2.COLOR_RGB2BGR)
        img_list.append(cam_images)
    # Plot image grid with labels
...
def plot_lr_scheduler(optimizer, scheduler, epochs=300, save_dir=''):
    ...
    for _ in range(epochs):
        scheduler.step()
        y_1.append(optimizer.param_groups[0]['lr'])
        y_2.append(optimizer.param_groups[1]['lr'])
        y_3.append(optimizer.param_groups[2]['lr'])
    x = range(0,300)
    plt.plot(x, y_1, x, y_2, x, y_3, '.-', label='LR')
    plt.xlabel('epoch')
    plt.ylabel('LR')
    plt.grid()
    plt.xlim(0, epochs)
...
@Timeout(100)  # known issue https://github.com/ultralytics/yolov5/issues/5611
...
def hough_line_det(mask_input):

    railway_lines = cv2.HoughLinesP(mask_input,
                                    rho=1,
                                    theta=np.pi / 360,
                                    threshold=10,
                                    minLineLength=30,
...
def feature_visualization1(features, model_type, model_id, feature_num=64):
    """
...
def feature_visualization_all(x, module_type, stage,  save_dir=Path('runs')):
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
